-- ============================================================================
-- Phase 1-4: 重複・不要テーブルの削除
-- ============================================================================
-- 目的: データベースをクリーンアップし、重複テーブルを削除
-- 実行方法: Supabase Dashboard > SQL Editor で実行
-- ============================================================================
-- 
-- 削除対象テーブル:
-- 1. badges (17行) → badge_definitions を使用
-- 2. follows (5行) → user_follows を使用
-- 3. likes (12行) → pin_likes + route_likes を使用
-- 4. favorites (7行) → route_favorites を使用
-- 5. official_routes (3行) → routes を使用
-- 6. follow_stats (7行) → RPC関数で動的計算
--
-- ============================================================================

-- ============================================================================
-- 1. badges テーブルの削除（badge_definitions に統合）
-- ============================================================================

-- badges テーブルが badge_definitions と重複しているか確認
DO $$
BEGIN
  ...
  ELSE
    RAISE NOTICE '✅ badges テーブルは既に削除されています';
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '✅ follows テーブルは既に削除されています';
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '✅ likes テーブルは既に削除されています';
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '✅ favorites テーブルは既に削除されています';
  END IF;
END $$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'official_routes') THEN
  ...
  ELSE
    RAISE NOTICE '✅ official_routes テーブルは既に削除されています';
  END IF;
END $$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'follow_stats') THEN
  ...
  ELSE
    RAISE NOTICE '✅ follow_stats テーブルは既に削除されています';
  END IF;
END $$;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'follow_stats') THEN
    v_deleted_count := v_deleted_count + 1;
    RAISE NOTICE '  ✓ follow_stats';
  END IF;
  
  RAISE NOTICE '';
  RAISE NOTICE '合計 % 個のテーブルを削除しました', v_deleted_count;
  RAISE NOTICE '';
END $$;
...
-- ============================================================================
-- End of Migration: 006_cleanup_duplicate_tables.sql
-- ============================================================================
