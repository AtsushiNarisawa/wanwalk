...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  related_pin_id UUID REFERENCES route_pins,
...
BEGIN
  ...
  SELECT 
    ...
    r.area_id,
    a.name AS area_name,
    ROUND((r.distance_meters / 1000.0)::NUMERIC, 2) AS distance_km,
    r.estimated_minutes,
    r.difficulty_level,
    r.total_pins,
  ...
  WHERE r.is_official = true
    AND (p_query IS NULL OR r.name ILIKE '%' || p_query || '%' OR r.description ILIKE '%' || p_query || '%')
    AND (p_area_ids IS NULL OR r.area_id = ANY(p_area_ids))
    AND (p_difficulties IS NULL OR r.difficulty_level = ANY(p_difficulties))
    AND (p_min_distance_km IS NULL OR (r.distance_meters / 1000.0) >= p_min_distance_km)
    AND (p_max_distance_km IS NULL OR (r.distance_meters / 1000.0) <= p_max_distance_km)
  ORDER BY
    CASE WHEN p_sort_by = 'popularity' THEN COALESCE(fav.favorites_count, 0) END DESC,
    CASE WHEN p_sort_by = 'distance_asc' THEN r.distance_meters END ASC,
    CASE WHEN p_sort_by = 'distance_desc' THEN r.distance_meters END DESC,
...
BEGIN
  ...
  -- ルートが存在するかチェック
  SELECT EXISTS(SELECT 1 FROM official_routes WHERE is_official = true LIMIT 1) INTO v_has_routes;
  ...
  ELSE
    SELECT id INTO route1_id FROM official_routes WHERE is_official = true LIMIT 1 OFFSET 0;
    SELECT id INTO route2_id FROM official_routes WHERE is_official = true LIMIT 1 OFFSET 1;
  ...
  RAISE NOTICE '   - test1@example.com / Test1234!';
  RAISE NOTICE '   - test2@example.com / Test1234!';
  RAISE NOTICE '   - test3@example.com / Test1234!';
  RAISE NOTICE '';
