...
// DogHub周遊コースのルートID
const ROUTE_ID = '8d0b7773-cac3-4f5b-938b-f21e92b4c0fe';
...
function saveGeometry(geometry) {
  const outputFile = `${__dirname}/route_geometry_doghub.json`;
...
async function main() {
  try {
    console.log('🗺️  DogHub周遊コースのジオメトリ再計算を開始...\n');
    ...
    if (result.features && result.features.length > 0) {
      ...
      const sqlFile = `${__dirname}/update_doghub.sql`;
